...
async def start_and_stream(
    ...
    if not input_video.exists():
        print(f"{RED}{BOLD}Error: Input video not found: {input_video}{RESET}\n")
        return

    # Create output directory if needed
    output_path.mkdir(parents=True, exist_ok=True)

    print(f"\n{BOLD}{CYAN}Starting video processing...{RESET}")
    print(f"  Input: {input_video}")
    print(f"  Output: {output_path}")
    print(f"  Duration: {duration}s")
    print(f"  Speedup: {speedup}x")
    print(f"  Title: {title or '(auto-generate)'}")
    print(f"  Music: {music_url or f'(auto-select from {music_tags})'}")
    print()

    # Suppress Prefect's console output but keep API logging
    # We'll show our own UI instead
    import os

    os.environ.setdefault("PREFECT_LOGGING_TO_API_ENABLED", "true")

    # Reduce console noise but don't completely silence (we need API logs)
    for logger_name in ["prefect.engine", "prefect.flow_runs", "prefect.task_runs"]:
    ...
    # Get flow runs before we start (to find ours)
    async with get_client() as client:
        runs_before = await client.read_flow_runs(limit=1)
        last_id_before = runs_before[0].id if runs_before else None
    ...
    # Wait a moment for flow to register
    ...
    async with get_client() as client:
        for _ in range(10):  # Try up to 10 times
            ...
            for run in runs:
                if last_id_before is None or run.id != last_id_before:
                    # This is likely our new flow run
                    if run.flow_id is not None:  # Sanity check
                        flow_run_id = run.id
                        break
            if flow_run_id:
                break
    ...
    if not flow_run_id:
        print(f"{YELLOW}Could not find flow run ID, waiting for completion...{RESET}\n")
        try:
            result = await flow_task
            print(f"\n{GREEN}{BOLD}✓ Processing complete!{RESET}")
            print(f"  Output: {result}\n")
        except Exception as e:
            print(f"\n{RED}{BOLD}✗ Processing failed: {e}{RESET}\n")
        return

    # Handle quiet mode
    if ui_mode and ("none" in ui_mode or "quiet" in ui_mode):
    ...
    print(f"{CYAN}Flow run ID: {flow_run_id}{RESET}")
    print(
        f"{DIM}View in Prefect UI: http://localhost:4200/runs/flow-run/{flow_run_id}{RESET}\n"
    )
    await asyncio.sleep(1)

    # Stream the flow run with our nice UI
    try:
        await stream_flow_run(
            flow_run_id, use_websocket=False, ui_mode=ui_mode
        ...
        print(f"\n{YELLOW}Streaming stopped: {e}{RESET}\n")
