...
def _create_rounded_title_image(
    ...
    """Create a PNG overlay: white rounded rect + black text. Returns path to temp file."""
    ...
    for line in lines:
        ...
        while line_w > inner_w and font_size > 20:
            font_size -= 2
            try:
                font = ImageFont.truetype(roboto_bold_path, font_size)
            except OSError:
    ...
    # Draw rounded rectangle (white background) within bounds
...
def add_title(
    ...
    overlay_path = _create_rounded_title_image(
        video_w, video_h, lines, height_percent, anchor
    )
...
async def main() -> None:
    ...
    # Parse height_percent
    height_percent = 10  # Default
    ...
    # Parse anchor (top or bottom — which edge of the box height_percent refers to)
    anchor = "top"  # Default (legacy)
    if "--anchor" in sys.argv:
        idx = sys.argv.index("--anchor")
        if idx + 1 < len(sys.argv):
            anchor = sys.argv[idx + 1].lower()
            if anchor not in ("top", "bottom"):
                print(f"Error: --anchor must be 'top' or 'bottom', got '{anchor}'")
                sys.exit(1)
    ...
    if not dry_run:
        add_title(video_path, title_text, output_path, duration, height_percent, anchor)
