...
def main():
    ...
    with col_settings:
        ...
        caption_height = st.slider(
            "Subtitle position (% from bottom)", 0, 100, 12, key="caption_height"
        )
    ...
    with col_steps:
        ...
        # Step 1: Remove Silence (unavailable: process_video removed)
        st.button(
            "1. 🔇 Remove Silence",
            key="btn_trim_silence",
            disabled=True,
            use_container_width=True,
            help="Unavailable: process_video was removed. Use propose_cuts + apply_cuts or interview_to_shorts.",
        )
        st.caption("Unavailable (process_video removed).")
        ...
        st.button(
            ...
            disabled=True,
            use_container_width=True,
            help="Unavailable: trim_smart was removed. Use propose_cuts + apply_cuts for segment planning.",
        )
        st.caption("Unavailable (trim_smart removed).")
        ...
        if st.button(
            ...
            output_path.parent.mkdir(parents=True, exist_ok=True)
            caption_height_val = st.session_state.get("caption_height", 12)
            cmd = [
                "python",
                "-m",
                "add_subtitles",
        ...
        if st.button(
            ...
            if not title:
                ...
                current_caption_height = st.session_state.get("caption_height", 12)
