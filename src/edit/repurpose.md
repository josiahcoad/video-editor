...
|---|---|---|---|
| **Video title** | Short text burned into the first 2-6 seconds of the video as a title card overlay. Search-optimised — should read like a real query someone would type into Google or ChatGPT. | `add_title.py`, `suggest_video_title.py` | "how to build trust with niche clients" |
...
**Key relationships:**
- Video title → burned into the video file (permanent, visual)
- Post title → metadata on the post record (editable, text-only)
- Video captions → burned into the video file (permanent, speech-synced)
- Post caption → metadata on the post record (editable, platform-specific body text)
...
// Or projects/<ClientName>/editing/settings.json (project-level; include business_id there)
{
  ...
  "captions": {
    ...
    "font_size": 22,            // ASS PlayRes units (20 ≈ 133px on 1920h)
  ...
  "propose_cuts": {
    ...
    "tolerance": 30             // ± seconds (default 30). CLI flags override.
  },
  "replacements": {             // spelling/caption fixes (transcription → display)
    "Markey": "Marky",          // common Deepgram mishearings
    "Marquee": "Marky"
  },
  "infographic": {
    "palette": {                // warm/approachable default — override per client
      ...
      videos/
        260208-organic-paid/
          ...
          outputs/
            merged-words.json
            segment_01/
              01_cut.mp4 → ... → 11_final.mp4
        260209-kai/
          inputs/
          outputs/
            kai-combined-words.json
            segment_01/
              01_cut.mp4 → ... → 06_ffmpeg_enhanced.mp4
  Anthony/
    editing/
      music/
        preferences.md
...
Scripts are standalone tools driven entirely by flags — they don't require a settings file. The settings file is a reference for the pipeline executor (human or agent) to know the project's preferences. If no `settings.json` exists, use reasonable defaults and ask the user.
...
**Do this first.** Many downstream scripts can transcribe on their own. For long-form source videos, pass existing transcripts via `--transcript` to avoid redundant API calls. For short-form segments (< 90s), letting scripts auto-transcribe is fine — it's cheap (~$0.01) and ensures timestamps match the actual video timeline (important after cuts or speedup).
...
3. Rate the cuts honestly (e.g. 6/10, 8/10) compared to what you would have done as an experienced editor. Consider:
   ...
   - **Segment opener coherence:** Read the FIRST 5-10 words of each segment aloud. Do they form a complete, self-contained statement? If the opener is a sentence fragment (e.g. "paid ads, social media ads, and organic" without the lead-in "I wanted to talk about the difference between..."), the cut is broken — a cold viewer's first impression will be confusion. Either extend the cut to include the full sentence or find a different start point that begins a new thought.
   - **No continuation-word openers:** The first words of every segment (and of every section within a segment, e.g. the body after a hook) must NOT be a conjunction or transition: "Also", "So", "And", "But", "Well", "Then", "However", etc. If the body starts with "Also, there are...", the cut is wrong — move the section start so it begins with "There are...". This is a mandatory QC item; fix cuts and re-run from apply_cuts if you find it.
   - **No duplicate phrases:** The same sentence or near-identical line must NOT appear twice in one segment (e.g. hook says "Everybody thinks you need a perfect credit score to buy a house" and body repeats "So a lot of people think you need a perfect credit score to buy a house"). If you hear the same idea twice, the cuts are wrong — trim the body start to after the duplicate so the viewer only hears it once. This is a mandatory QC item; fix cuts and re-run from apply_cuts if you find it.
...
5. **Wait for the user's decision:**
   ...
   - **Iterate** — re-run `propose_cuts` with adjusted parameters (different `--duration`, `--tolerance`, `--count`, or `--model`) to improve the result. Use `--prompt` only as a last resort when the LLM consistently misunderstands the content focus. Then re-review.
...
## Phase 2b: Cut Text Verification (MANDATORY — before touching ffmpeg)
...
```python
# Quick verification script — run for every proposed cut
import json
...
2. **No duplicate phrases:** Read the full text of the segment (all sections concatenated). Does the same sentence or near-identical line appear twice (e.g. hook and body both saying "everyone thinks you need a perfect credit score to buy a house")? If yes, trim the later section's start so the duplicate is removed — the viewer must only hear the idea once.
...
> **Root cause this prevents:** Picking timestamps from word-chunk boundaries or 15-word groups without verifying they land on sentence boundaries. The downstream pipeline (speedup, jump cuts) cannot fix a cut that starts mid-sentence — it only makes the audio faster and adds zoom effects. Garbage in, garbage out.
...
# and most reliable approach — timestamps match the actual video exactly.
#
...
> **Legacy fallback:** If you do pass `--transcript` and `--speedup`, the script still supports the old path and will remap timestamps using `source_ranges` from the boundaries sidecar. But prefer auto-transcription — it's simpler and avoids coupling between scripts.
...
5. **Suppresses zoom toggles within ±1s of content-cut boundaries** (from `apply_cuts`) to prevent "double jumps" where a content discontinuity and a zoom change happen simultaneously
...
- Use OpenCV face detection with adaptive sampling: start at 10s intervals, if face moves > threshold, subdivide (5s → 2.5s → 1s → 0.5s → 0.1s) to track movement precisely
...
**Always use ffmpeg `filter_complex` concat** with `fps=30` normalization — never stream copy (`-c copy` or `compose_clips.py` fast mode). The hook and main segment will almost always have different framerates after speedup, and stream copy concat silently produces incorrect durations.
...
- If the user doesn't want to re-record — fall back to a cold open (pulled from the existing video) or skip the hook entirely
...
**Run this BEFORE title and captions.** Voice enhancement changes the audio, and the caption transcript must come from the final audio. Pipeline order: jump cuts → enhance → (isolate) → title → transcribe → captions.

### Phase 4 alt: Voice Isolation (optional — only when requested)
...
- More aggressive than the ffmpeg chain — strips everything that isn't voice.
- If adding music later, voice isolation helps the voice stay clean against the music track.
- Run this **before** title/captions — same reason as enhancement: the transcript must match the final audio.
...
```bash
# Standard (white box, black text — use values from settings.json if available):
...
# Split-screen screen demo (black box, white text — title centered in frame):
python src/edit/add_title.py <segment.mp4> "Title Text Here" <output.mp4> \
  --duration 6 --anchor center --dark --voice <client>/editing/voices/marky_default.md
```

- Read `settings.json` → `title.duration`, `title.height_percent`, `title.anchor` and pass as flags.
...
- `--transcript` — path to the word-level transcript JSON **from the titled video**. The script auto-discovers the sibling `*-utterances.json` for smart captioning (splits at punctuation).
...
After burning in captions, **run `preview_chrome.py` at t≈4s and t≈10s, then view both output images** (the agent can read image files). Do not delegate this to the user. Verify:
1. **t≈4s** — title + captions both visible. Check they don't overlap each other or the face, and are above platform chrome.
2. **t≈10s** — captions only (title has faded). Check captions are clear, well-positioned, and not in the chrome zone.

**Face-overlap check (the #1 caption failure):**
- Look at the extracted frames. Are the captions covering ANY part of the speaker's face — chin, jaw, mouth, or neck?
...
## Phase 6b: Emoji Overlays (optional — when content benefits from visual emphasis)
...
```bash
# Dry-run first (recommended) — review picks before committing to a render:
...
- `--transcript` accepts a word-level JSON. If omitted, the script transcribes the video automatically (wasteful if you already have one — always pass it).
- Emojis are 240×240px with a semi-transparent white circular backing for visibility against any background.
...
- **Talking-head explainers** where the speaker mentions concrete nouns, numbers, or strong concepts — emojis anchor the idea visually.
- **Short-form content** where visual variety keeps attention — emojis act as mini-cuts without needing b-roll.

### When to skip

- **Screen recordings / product demos** — the screen content IS the visual. Emojis would distract.
- **Emotional / serious content** — emojis undercut gravity.
- **Videos that already have infographics or heavy overlays** — adding emojis on top feels cluttered. Pick one or the other.

### QC

- **Dry-run review:** Read each pick's rationale. Does the emoji genuinely reinforce the spoken word, or is it forced? Reject picks that are generic (e.g., 🔥 on a non-emphatic moment).
...
## Phase 6c: Infographic Overlay (optional — when a concept benefits from visual explanation)
...
```bash
# Dry-run — review concept, template, timing, and preview image:
...
- `--dry-run` saves a preview PNG to `/tmp/infographic_preview.png` and prints the plan as JSON. **Always dry-run first** — inspect the image and verify the concept/timing make sense before a full encode.
...
|----------|----------|---------|
| `funnel` | Processes, pipelines, cause→effect flows | Paid Ads → Awareness → Trust → Purchase |
...
- **Explainers** where the speaker describes an abstract concept (trust gap, sales funnel, customer journey) — the graphic anchors the idea.
- **Comparisons** where the speaker contrasts two approaches — seeing them side-by-side is clearer than hearing them sequentially.
- **Key statistics** that are more impactful when displayed visually.

### When to skip

- **Content that's already visual** (screen recordings, product demos) — the screen IS the graphic.
- **Videos under 20s** — an infographic overlay would dominate too much of the runtime.
- **No clear concept** — if the video is conversational/anecdotal without a structured concept, forcing an infographic feels artificial.
...
### Short-version trim (35 ± 10s)

Create a **shorter cut** of each segment’s final infographic video for platforms that favor ~35s. Use **target 35s, tolerance 10s** (acceptable range 25–45s). Do **not** use 30s; 35 ± 10 is the spec.
...
```bash
# Basic — single caption for all platforms.
...
# With overrides from files (@ prefix reads from file — useful for long captions):
python src/edit/schedule_post.py \
  --video <final.mp4> \
...
- **Business ID:** Resolved in order: `--business_id` → project `editing/settings.json` (when video path is under `projects/<Client>/editing/videos/...`) → `MARKY_BUSINESS_ID` env var. Put `business_id` in each project's `editing/settings.json` so you don't need to pass it or set the env per run.
...
`--override PLATFORM CAPTION` creates a row in `post_channel_overrides` — one video post, different captions per platform. This is repeatable (use multiple `--override` flags).
...
```bash
# Option A: Batch mode — per-platform voice files, one command:
...
The base `--caption` is used as the default for any platform without an override. Overrides only replace the caption — media (video) is shared across all platforms.

**Answer The People (ATP) for copy:** To optimize captions for real search demand (like the ideate script does for topics), use **`suggest_video_title.py`** — it calls the same AnswerThePublic API as `src/ideate/ideate.py`. Run it once on the full or segment transcript, save the output, then pass that file to `write_copy.py` as `--search-context`:
...
`--search-context` also accepts a plain JSON array of strings (hand-written phrases) if you don’t run the ATP script.
...
PRE-PRODUCTION (per client, done once):
  A.  Profiler           Follow src/profiler.md → produce profile.md (stable) + strategy.md (content strategy, active)
  B.  Locate             src/locate/idea_generator.py --profile brand_profile.json → topic ideas

PRODUCTION (per recording session — scripts in src/edit/):
  0.  Read settings        cat <project>/settings.json (if it exists — pass values as flags below)
  0b. Download source      yt-dlp (if YouTube URL — see Phase 0)
  ...
  14. Generate text posts  write_copy.py --platform linkedin/twitter/facebook → schedule_post.py
  15. Generate carousel    gen_carousel.py → upload → schedule_post.py
```
...
| Propose cuts | Segment count, durations (25-55s), cohesion, hook quality, **opener coherence** | Read the JSON output. Does each segment tell a complete story? **Read the first 5-10 words of each segment aloud — they must be a complete, self-contained statement, not a mid-sentence fragment.** Would YOU watch it? |
| **Cut text verification** | **Every section starts at a sentence boundary, ends cleanly, no stray words** | **Run Phase 2b text extraction for every proposed cut. Read the first and last 5 words of each section. This is the #1 failure point for cuts — more important than visual QC.** |
...
| Voice enhancement | Voice sounds warmer and fuller, no distortion or artifacts | Listen to a few seconds. Compare against the raw version — should sound noticeably less tinny/echoey. |
...
| Add title | Title is NOT on the face, NOT clipped by platform chrome, readable | **Run `preview_chrome.py` at t≈1-2s, then view the output image.** Confirm title is in safe zone and not on face. This is the #1 failure point. |
| Add captions | Captions sit on chest (BELOW chin), don't overlap title, words are correct | **Run `preview_chrome.py` at t≈4s and t≈10s on the captioned video, then view both output images.** Confirm captions are below the chin line and not in chrome. If any face overlap, lower `--height` by 5-10% and re-render before proceeding. |
...
**Do not consider a segment (or the full repurpose run) complete until you have verified the final video's transcript.** This is the agent's responsibility — the user should not have to catch cut-off sentences, jumbled clip order, or wrong hooks.
...
2. **Read the full transcript** (the `.txt` file). Verify:
   - **Coherent narrative:** The order of sentences makes sense. No jump from topic A to unrelated topic B and back (e.g. "next gen buyers" → "oh there's my video, my Marquee helps me" → "they're going to Instagram").
   - **Clean start / hook at start:** The first sentence (or first 5–10 words) is the intended hook or a proper sentence start — not a conjunction or transition ("Also", "So", "And", "But") and not a mid-sentence fragment. If the segment opens with "Also, there are..." after the hook, the cuts are wrong: fix the body start timestamp and re-run from apply_cuts for that segment.
   - **No duplicate phrases:** The same sentence or near-identical line must NOT appear twice (e.g. "everyone thinks you need a perfect credit score to buy a house" said in the hook and again in the body). If you hear the same idea twice, the cuts are wrong — trim the body start to after the duplicate and re-run from apply_cuts for that segment.
...
**Agent instruction: you can view image files.** When running title/caption QC, **you must run the chrome preview yourself and then open/view the generated images** to verify placement. Do not ask the user to inspect — do the inspection as part of the pipeline.
...
1. **Run** `preview_chrome.py` at t≈4s and t≈10s for each captioned segment (or at least one representative segment).
...
```bash
# From the video-editor repo root — preview_chrome.py lives at repo root
# Frame with title + captions (t≈4s): extracts frame, draws chrome zones, saves <stem>_chrome.jpg
...
**Fallback — plain frame only:** Use `extract_frame.py` if you only need a raw frame without chrome overlay:
...
Cursor can open and play `.mp4` files directly in the editor. This is useful for full QC when frame extraction isn't sufficient (e.g., checking pacing, transitions, dog footage). **Only use this when the user explicitly asks** — don't proactively open videos in the IDE. For routine QC, frame extraction is enough.

### Video description for visual content

For videos where visual content matters (not just talking head — e.g., pets, product demos, b-roll), use `describe_video.py` to get a per-second visual description before deciding cuts:
