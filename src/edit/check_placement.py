...
def parse_placement_text(text: str) -> dict[str, int | str]:
    """Parse LLM placement output into TITLE_HEIGHT_PERCENT, TITLE_ANCHOR, CAPTION_HEIGHT_PERCENT.

    Returns dict with keys title_height_percent (int), title_anchor (str), caption_height_percent (int).
    Uses defaults if parsing fails.
    """
    import re
    ...
    for line in text.splitlines():
        line = line.strip()
        if line.startswith("TITLE_HEIGHT_PERCENT:"):
            m = re.search(r"(\d+)", line)
            if m:
                out["title_height_percent"] = int(m.group(1))
        elif line.startswith("TITLE_ANCHOR:"):
            if "bottom" in line.lower():
                out["title_anchor"] = "bottom"
            elif "center" in line.lower():
                out["title_anchor"] = "center"
            else:
                out["title_anchor"] = "top"
        elif line.startswith("CAPTION_HEIGHT_PERCENT:"):
            m = re.search(r"(\d+)", line)
            if m:
                out["caption_height_percent"] = int(m.group(1))
    return out
...
def get_placement(
    ...
    """Run LLM placement analysis and return parsed title/caption placement.

    Returns dict with title_height_percent, title_anchor, caption_height_percent.
    """
    if timestamps is None:
        timestamps = [0, 3, 10]
    text = analyze_placement(video_path, timestamps, model)
